...
Our study area is rather small, covering a 4 by 4 km forest area northeast of Marburg. The core part of the AOI is the Marburg Open Forest (MOF) a field research facility of the Philipps University Marburg.
...
To get an overview of the data, we can compute simple summary statistics applying `reducer` functions over dimensions. Using the `count` reducer we learn about the temporal coverage for each aggregated pixel and hence an initial understanding of the image collection.

```{r count}
ncdf_cube(file.path(tutorialDir,"lahntal.nc")) |>
  reduce_time("count(B04)") |> 
  plot(key.pos = 1, zlim=c(35,45), col = viridis::viridis, nbreaks = 10)
```

 We can see that most time series contain valid observations for 35-45 months, which should be sufficient for our example. Similarly, it is also possible to reduce over space, leading to summary time series.
...
tmap_arrange(tm_ndvi,ncol = 2,nrow=2 ,asp = NA)
tmap_arrange(tm_kndvi,ncol = 2,nrow=2 , asp = NA)
