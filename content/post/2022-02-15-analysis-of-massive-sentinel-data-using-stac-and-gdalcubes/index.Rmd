...
One major challange is the fact that most of the earth surface related remote sensing activities are heavily *"disturbed"* by the atmosphere, especially by clouds. So to find cloud free satellite imagery is a common and cumbersome task. This task is supported by the `rstac` package which provides a convenient tool to find and filter adequate Sentinel-2 images out of the COG data storage. However, to address the AOI we need to provide the extend via the `bbox` argument of the corresponding function `stac_search()`. So first we  need to derive and transform the required bounding box to WGS84 geocoordinats, easily done with the `sf` functions  `st_bbox()` and `st_transform()`. In addition we adapt the projection of the referencing vector objects to all other later projection needs. 
...
# call background map
osm_forest <- read_osm(forest_mask,type = "bing")
...
ncdf_cube(file.path(tutorialDir,"MOF_10.nc")) |>
  apply_pixel("(B08-B04)/(B08+B04)", "NDVI") |>
  zonal_statistics(forest_32632,
                   expr = "median(NDVI)",
                   out_path = file.path(tutorialDir,"MOF_10.gpkg"),
                   overwrite = TRUE,  as_stars = TRUE) |>
  plot(max.plot = 48, col = ano.col, nbreaks=8)
...
mask <- gdalUtils::gdal_rasterize(src_datasource=paste0(tutorialDir,"forest/MOF_mask.shp"),
...
#tmap_arrange(tm_ndvi,ncol = 1,asp = NA,widths = c(0.33,0.33,0.33),outer.margins = 0.001)
```

```{r tm_ndvi,  messages=FALSE, warnings=FALSE}
...
```{r plot_bfast, messages=FALSE, warnings=FALSE}
...
m2 =  tm_shape(osm_forest) + tm_rgb() +
  ...
  tm_layout(legend.title.size = 1,
            ...
            legend.outside = TRUE) +
  tm_grid() 
#tmap_arrange(m1,m2,asp=NA,nrow = 2)
m1
m2
```

Running `bfastmonitor()` is computationally expensive. However, since in common (not in our case) the data is located in the cloud, it would be obvious to launch one of the (payed) more powerful machine instance types with many processors. Parallelization within one instance can be controlled entirely by `gdalcubes` using `gdalcubes_options()`.

The visualisation is pretty straighforward. A typical pip is save a resulting netCDF file, convert it to a `stars` object, and finally use the `tmap` or `mapview` package to create an (interactive) map(s).     

From a scientific point of view we need to tune some aspects. Teh kNDVI Index together with the bfastmonitor approach is somewhat ssophisticated and need a validation strategy. Also the correlation to the different treespecies could be promising. So there is certainly a need for more analysis to understand the processes and to identify false results.

# Summary

This example has shown, how simple and more more complex methods can be applied on data cubes. Especially the choosen aproach to "download" the data and perform the processing on the local machine is kond of promising due to the fact that the code can be run almost identically on a cloud instance. The use of more common or well know functionalities and packages helps a lot migrating to gdalcubes concept.

It is even more powerful to use this workflow for extremly comfortable multisensor multiscale approaches. 
