...
In der Geo- oder Umweltinformatik ist die Erfassung von Landoberflächen-Veränderungen mittels Satelliten-, Flugzeug- oder Drohnenbildern, die sogenannte Change Detection Analysis ein wichtiger Anwendungsfall. Oft werden solche Ergebnisse mit bio- und geophysikalischen oder anthropogenen Prozessen in Verbindung gebracht, um ein sowohl ein tieferes Verständnis als auch die Möglichkeit zur Entwicklung von Vorhersagemodellen zu erhalten. Um räumliche Informationen aus den zugrundeliegenden Prozessen zu erzeugen sind Bildanalysemethoden von hervorgehobener Bedeutung. Da diese Bildaten ein immens wachsender Bereich sind, ist es zunehmend notwendig sog. Big Data Konzepte in die Analysen zu integrieren, also mit enormen Datenmengen automatisiert und nachvollziehbar umzugehen.

Dies ist sowohl für den wissenschaftlichen Erkenntnisgewinn als auch für gesellschaftliche Zukunftsaufgaben zwingend. Wir starten wie bereits einleitend ausgeführt mit einer skalierbaren Change Detection Analyse von Waldschäden in Mittelgebirgen, also einer typischen anwendungsorientierten Aufgabe.
...
root_folder = "/media/creu/meta/active/geoinfo"
...
Wichtig sind sind die beiden Befehle mit Vorgangsnummern: 
Mit dem ersten Befehl kann gecheckt werden ob die Daten im Zugriff sind. Dies dauert in der Regel nicht länger als 30 Minuten. Mit dem zweiten Befehl können Sie dann heruntergeladen und prozessiert werden. Bitte beachten Sie dass dies eine gute Internetverbindung vorraussetzt. 
...
Sobald der Download erfolgreich war kann mit der eigenlichen Klassifizierungsarbeit begonnen werden. Zuerst müssen die Daten in geeigneter Weise nämlich als ein Multikanalbild organisiert werden. Zusätzlich wird eine aktuelle Corine Landnutzung geladen, um eine binäre Wald/nicht Wald Maske zu erzeugen. Für den [Download der Corine Daten](https://land.copernicus.eu/pan-european/corine-land-cover) kann das bereits verfügbare Copernicus Konto genutzt werden. Nach dem (manuellen) Download die Daten in das Verzeichnis `level0`  Verzeichnis entpacken (Achtung hier gibt es nach dem entpacken Archivs zahllose Unterverzeichnisse wir benötigen die Datei `U2018_CLC2018_V2020_20u1.tif`. Alternativ kann der bereits korrekt prozesierte und projizierte Datensatz vom github repository (siehe Skript) genutzt werden.
...
# Modell
prediction_kmeans_2019 = unsuperClass(pred_stack_2019, nClasses = 5,norm = TRUE, algorithm = "MacQueen")
# Klassifikation
mapview(prediction_kmeans_2019$map)

prediction_kmeans_2020 = unsuperClass(pred_stack_2020, nClasses = 5,norm = TRUE, algorithm = "MacQueen")
mapview(prediction_kmeans_2020$map)
```
...
m2 = tm_shape(pred_stack_2019) + tm_rgb(r=4, g=3, b=2) +
  tm_layout(legend.outside.position = "right",
            legend.outside = T,
            panel.label.height=0.6,
            panel.label.size=0.6,
...
Als Resultat liegt nun eineTabele mit den Trainingsdaten  für 2019 und 2020 vor. Der Datensatz beinhalten alle Rasterinformationen über alle Bänder die von den Polygonen für die Klassen *"clearcut"* und *"other"* abgedeckt werden. 
...
idx = createDataPartition(tDF$class,list = FALSE,p = 0.15)
trainDat = tDF[idx,]
testDat = tDF[-idx,]
...
prediction_mlc_2019       <- superClass(pred_stack_2019, trainData = sp_trainDat[,1:16],valData = sp_testDat[,1:16], responseCol = "class", model = "mlc", tuneLength = 1, trainPartition = 0.7,verbose = TRUE)

prediction_mlc_2020       <- superClass(pred_stack_2020, trainData = sp_trainDat[,1:16],valData = sp_testDat[,1:16], responseCol = "class",model = "mlc", tuneLength = 1, trainPartition = 0.7,verbose = TRUE)
```
...
Bei qualitativen Vergleich fällt auf dass Random Forest und Maximumlikelihood Klassifikation vergleichbare Ergebnisse liefern.  


```{r vergleichkalssifikationen}

## ---- Visualisierung mit mapview ----
mapview::viewRGB(mask*pred_stack_2020, r = 4, g =3, b = 2,maxpixels =  1693870)+
...
Betrachten sie sie nachfolgenden Ressourcen als Beispiele dafür, wie aus solchen vielfältig verfügbaren Ressourcen eine bestimmte technische und konzeptionelle Vorgehensweise zur Bearbeitung einer Fragestellung *"entsteht"*. Nach viel Recherche und kritischer Gegenprüfung wird ein aktuell als gesichert betrachter Stand innerhalb der wissenschaftlcihen Gemeinschaft identifiziert und der eine gute Grundlage guter wissenschaftlicher Praxis darstellt. 
