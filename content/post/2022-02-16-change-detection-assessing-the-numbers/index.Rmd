...
tags:
  - Remote Sensing
subtitle: ''
description: 'Da in jeder Klassifizierung von Daten immer Fehlklassifikationen vorkommen und die Ursache multikausal ist ist die Bewertung ein entscheidner Schritt um die Belastbarkeit der Kalssifikation einschätzen zu können.'
...
Da in jeder Klassifizierung von Daten immer Fehlklassifikationen vorkommen und die Ursache multikausal ist ist die Bewertung ein entscheidner Schritt um die Belastbarkeit der Kalssifikation einschätzen zu können. 
...
Vor allem in der Fernerkundung und der räumlichen Analyse kategorialer Daten sind die sogenannten Kappa-Indizes das Standardmaß  eine Klassifizierung (Vorhersage) einer Karte Referenzkarten (Beobachtung/andersartige Beobachtung) zu vergleichen. Allgemeiner ausgedrückt, zwei kategoriale Datensätzen miteinander zu vergleichen. Es gilt hierbei die Annahme, dass sowohl die Quantität als auch die räumliche Lage jeder Klasse zufällig sind. Basierend auf der obigen Kreuztablle werden die Kappa Werte wie folgt berechnet:


$$
PA= \sum_{i=1}^3 p_{i,i}
$$

$$
PE= \sum_{i=1}^3 pT_i*Tp_i
$$



$$
Pmax= \sum_{i=1}^3 min(pT_i,Tp_i)
$$
...
Jedoch gibt es begründete Kritik an dieser üblichen Vorgehensweise  (@Pontius_Milliones2011). Sie schlagen vor aus der Kreuztabellenmatrix die Parameter *Mengenabweichung* und *Zuordnungsabweichung* zu berechnen Tatsächlich ermöglichen diese Parameter eine gute Einschätzung der Mengen- und Lagequalität der Übereinstimmung kategorialer Daten.
