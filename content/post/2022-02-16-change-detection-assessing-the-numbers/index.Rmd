...
Jedoch schlagen (@Pontius_Milliones2011) als optimale Vorgehensweise vor, aus der Kreuztabellenmatrix die Parameter *quantity disagreement* (QD) und *allocation disagreement* (QD) zu berechnen Tatsächlich ermöglichen diese Parameter eine gute Einschätzung der Mengen- und Lagequalität der Übereinstimmung kategorialer Daten.
...
Die Konfusionsmatrix besteht in diesem Fall korresponiderend źur Vierfeldtabelle aus vier binären Rastergrids. Sollen die Klassenwechsel *von* und *zu* visuell analysiert werden müssen die Klassifikationskarten entsprechend aufbereitet werden. Das nachstehende Skript identifiziert für jedes Pixel die Referenzklasse (erstes Bild)  und die im zweiten Bild beobachtete Klassenveränderung. Diese werden korrespodierend relassifiziert und als Rasterkarten ausgegeben. 
...
derselben Auflösung und Ausdehnung gelesen sind unsere Klassifikationsdatensätze. 2019 und 2020. Wir nutzen 
 Mit der Funktion `lsp_compare()` wird ein Co-Occurrence-Vektor als Signatur verwendet und der Jensen-Shannon-Abstand als Ähnlichkeitsmass. Für unser Beispiel haben wir eine Region von 25 Zellen im Quadrat (250**m).
 
Das resultierende Objekt gibt einen Überblick über die Veränderungen der Waldbedeckung im Nrdwest-Harz.
