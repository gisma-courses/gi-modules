...
We make a short general check that we can start over with the real processing, i.e. we check if everything is ready to use. First load the tutorial data to an appropriate data folder of your project. Then add the following libraries.


```r
# define  additional packages comment if not needed
appendpackagesToLoad = c("lidR", "mapview", "raster", "rlas", "sp", "sf")
```
...
plot(lidar_file, bg = "green", color = "Z",colorPalette = mvTop(256),backend="rgl")
```
